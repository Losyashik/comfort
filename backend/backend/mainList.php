...
if (!in_array(13, $rights)) {
  if (isset($_GET['limit'])) {
    ...
    $orderList = $connect->getData("SELECT * FROM orders WHERE id_status in (1,2,3,4,8,11$prohibition) AND id in (SELECT id_order FROM order_history WHERE id_user = " . $_SESSION['user']['id'] . ") ORDER BY id DESC");
  }
} else {
  if (isset($_GET['limit'])) {
    ...
    $orderList = $connect->getData("SELECT * FROM orders WHERE id_status in (1,2,3,4,8,11$prohibition) ORDER BY id DESC");
  }
}
...
if (count($orderList) != 0) {
  ...
  $mainList['list'] = array(array("text" => "Не добавлено ни одной заявки"));
}
