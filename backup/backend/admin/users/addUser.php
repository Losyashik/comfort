...
if ($connect->query("SELECT * FROM users WHERE login = '" . $_POST['login'] . "'", "result")->num_rows == 0) {
    if ($connect->query("SELECT * FROM users WHERE name = '" . $_POST['name'] . "'", "result")->num_rows == 0) {
        $login = $_POST['login'];
        $name = $_POST['name'];
        $password = password_hash($_POST['password'], 0);
        $connect->query("INSERT INTO users (name,login,password) VALUES ('$name','$login','$password')");
